...
//	char str[100];
	...
	if (m_pPDP->TrackFlag == 1)
	{
		...
		sprintf(outputdir_spec_dtn, "%s/%s", outputdir, "DTN");
		sprintf(outputdir_spec_dtnba, "%s/%s", outputdir, "DTNBA");
		sprintf(outputdir_spec_dtnbio, "%s/%s", outputdir, "DTNBIO");
		sprintf(outputdir_agerange_spec_dtn_all, "%s/%s", outputdir, "SPEC_AGERANGE");

		if (_mkdir(outputdir_spec_dtn) != 0)
		{
			if (errno != EEXIST)
				errorSys("Harvest: Can't create the dead tree number in diff spec direcory", STOP);
		}
		if (_mkdir(outputdir_spec_dtnba) != 0)
		{
			if (errno != EEXIST)
				errorSys("Harvest: Can't create the dead tree basalarea in diff spec direcory", STOP);
		}
		if (_mkdir(outputdir_spec_dtnbio) != 0)
		{
			if (errno != EEXIST)
				errorSys("Harvest: Can't create the dead tree biomass in diff spec direcory", STOP);
		}
		if (_mkdir(outputdir_agerange_spec_dtn_all) != 0)
		{
			if (errno != EEXIST)
				errorSys("Harvest: Can't create the dead tree number&biomass&basalarea in diff spec&agerange direcory", STOP);
		}
...
//	pstands = Stands;
	...
	pHarvestsites = new HARVESTSites(giRow, giCol,pdp->TrackFlag);
...

{
	...
	if  (iset == 0) {
		...
		do {
			...
			v1 = (float)2.0*frand1() - (float)1.0;

			v2 = (float)2.0*frand1() - (float)1.0;
...
//    IntArray sumCut(stands.number());		Modified by Vera
    ...
    if (firstTime) {
		...
		fprintf(harvestOutputFile1, "decade\tmanagementArea\tstand\tnumSitesHarvested\n");
...
void PutOutput_harvestBACut(char *fn, char *fn1, double wAdfGeoTransform[]){
	...
	//pHarvestsites->clearValueHarvestBIO_spec();				//add by houbin 11/2/2017
	//pHarvestsites->clearValueHarvestNum_spec();				//add by houbin 11/2/2017
...
void PutOutput_harvestNumCut_spec(char *fn, char *fn1, int spec, double wAdfGeoTransform[])			//add by houbin 10/27/2017
{
	FILE *fpOutput;
	...
	poDstDS = poDriver->Create(fn1, pCoresites->numColumns(), pCoresites->numRows(), 1, GDT_UInt32, papszOptions);//*
	...
	for (i = pCoresites->numRows(); i>0; i--){
		...
		for (j = 1; j <= pCoresites->numColumns(); j++){
			...
			*(pintScanline + (pCoresites->numRows() - i)*pCoresites->numColumns() + j - 1) = pHarvestsites->GetValueHarvestNum_spec(i, j, spec);//* 
...
void PutOutput_harvestBioCut_spec(char *fn, char *fn1, int spec, double wAdfGeoTransform[])			//add by houbin 10/27/2017
{
	FILE *fpOutput;
	...
	for (i = pCoresites->numRows(); i>0; i--){
		...
		for (j = 1; j <= pCoresites->numColumns(); j++){
			...
			*(pafScanline + (pCoresites->numRows() - i)*pCoresites->numColumns() + j - 1) = pHarvestsites->GetValueHarvestBio_spec(i, j, spec);//* 
...
void PutOutput_harvestBACut_interval(char *fn, char *fn1, int interval_index, double wAdfGeoTransform[]){
	...
	for (i = pCoresites->numRows(); i>0; i--){
		...
		for (j = 1; j <= pCoresites->numColumns(); j++){
			...
			*(pafScanline + (pCoresites->numRows() - i)*pCoresites->numColumns() + j - 1) = pHarvestsites->GetValueHarvestBA_interval(i, j, interval_index);//* 
		}
...
void PutOutput_harvestBIOCut_spec_interval(char *fn, char *fn1, int spec_index, int interval_index, double wAdfGeoTransform[]){
	...
	for (i = pCoresites->numRows(); i>0; i--){
		...
		for (j = 1; j <= pCoresites->numColumns(); j++){
			...
			*(pafScanline + (pCoresites->numRows() - i)*pCoresites->numColumns() + j - 1) = pHarvestsites->GetValueHarvestBIO_spec_interval(i, j, spec_index, interval_index);//* 
...
void PutOutput_harvestBACut_spec_interval(char *fn, char *fn1, int spec_index, int interval_index, double wAdfGeoTransform[])//add by houbin 10/27/2017
{
	FILE *fpOutput;
	...
	for (i = pCoresites->numRows(); i>0; i--){
		...
		for (j = 1; j <= pCoresites->numColumns(); j++){
			...
			*(pafScanline + (pCoresites->numRows() - i)*pCoresites->numColumns() + j - 1) = pHarvestsites->GetValueHarvestBA_spec_interval(i, j, spec_index, interval_index);//* 
...
void PutOutput_harvestNumCut_spec_interval(char *fn, char *fn1, int spec_index, int interval_index, double wAdfGeoTransform[])//add by houbin 10/27/2017
{
	FILE *fpOutput;
	...
	for (i = pCoresites->numRows(); i>0; i--){
		...
		for (j = 1; j <= pCoresites->numColumns(); j++){
			...
			*(pintScanline + (pCoresites->numRows() - i)*pCoresites->numColumns() + j - 1) = pHarvestsites->GetValueHarvestNum_spec_interval(i, j, spec_index, interval_index);//* 
...
void Clear_NUM_BIO_BA_spec_interval()
{
	pHarvestsites->clearValueHarvestNUM_BA_BIO();
}