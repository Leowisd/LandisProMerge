...
void HarvestRegime::writeReport(FILE* outfile) {
...

	{
		...
		fprintf(outfile, "eventType\tdescription\tdecade\tmanagementArea\tsitesCut\tsumAgesCut");



        for (int spp = 1; spp <= pspeciesAttrs->number(); spp++)

			fprintf(outfile, "\t%s", (*pspeciesAttrs)(spp)->name);
...
/* Changed isA() to getSequentialId() */
	...
	fprintf(outfile, "%d\t%s\t%d\t%d\t%d\t%d", getSequentialId(), getLabel(), currentDecade, getManagementAreaId(),
...
/*  for (int spp = 1; spp <= speciesAttrs.number(); spp++)           Modified by Vera*/
    ...
    for (int spp = 1; spp <= pspeciesAttrs->number(); spp++)
		...
		fprintf(outfile, "\t%d", itsReport->sumOfCohortsCut(spp));
